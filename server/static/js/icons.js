// icons.js — Lucide(ISC license, https://lucide.dev) inline SVG 아이콘.
// 런타임 의존성 0: CDN/패키지 없이 path 데이터를 인라인으로 박아 SVG를 만든다.
// 색은 stroke=currentColor 라서 부모 텍스트 색을 따른다. 크기는 1em(부모 font-size).

const SVG_NS = "http://www.w3.org/2000/svg";

// 각 항목은 Lucide 아이콘의 inner SVG (viewBox 0 0 24 24, stroke 기반).
const PATHS = {
...
// icon("camera", {cls}) → <svg class="ic ...">. 부모 font-size/color를 따른다.
...
// 정적 마크업(탭바 등)의 data-icon 플레이스홀더를 실제 SVG로 "치환"한다.
// (자식으로 넣지 않고 교체 → .ic 중첩으로 인한 변형 중복 방지)
export function hydrateIcons(root = document) {
  root.querySelectorAll("[data-icon]").forEach((host) => {
    if (host.tagName.toLowerCase() === "svg") return; // 이미 치환됨
    const extra = host.className.replace(/\bic\b/g, "").trim();
    host.replaceWith(icon(host.dataset.icon, extra ? { cls: extra } : {}));
  });
}
